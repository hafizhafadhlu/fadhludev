...
def delete_command():
    p12backend.delete(selected_row[0])

def view_command():
    list.delete(0,END)
    for row in p12backend.view():
        list.insert(END,row)

def search_command():
    list.delete(0,END)
    for row in p12backend.search(Date_text.get() , Earnings_text.get() , Exercise_text.get() , Study_text.get() , Game_text.get() , Python_text.get()):
        list.insert(END,row)

def add_command():
    p12backend.insert(Date_text.get() , Earnings_text.get() , Exercise_text.get() , Study_text.get() , Game_text.get() , Python_text.get())

    list.delete(0,END)
    list.insert(END,(Date_text.get() , Earnings_text.get() , Exercise_text.get() , Study_text.get() , Game_text.get() , Python_text.get()))

win = Tk()

win.wm_title('SUKA-SUKA GUE')
...
Game_text = StringVar()
e5 = Entry(win , textvariable=Game_text)
